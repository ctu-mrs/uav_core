...
'
  'uvdar_core' 'waitForRos; waitForOdometry; roslaunch uvdar start_dataset_1t.launch
  '
  'uvdar_kalman' 'waitForRos; waitForOdometry; roslaunch uvdar uvdar_kalman.launch
  '
  'uvdar_reprojection' 'if [ $TEST -ne "true" ]; then waitForRos; waitForControl; fi; roslaunch uvdar reprojector.launch'
  # 'Realsense' 'waitForRos; roslaunch realsense_d435 high_acc.launch'
  # 'Detect' 'waitForRos; roslaunch object_detect object_detect.launch'
  # 'Planner' 'waitForRos; roslaunch balloon_planner choose_balloon.launch'
  # 'Detection' 'waitForRos; roslaunch balloon_planner localization_pipeline.launch'
  # 'Destroyer' 'waitForRos; roslaunch balloon_circle_destroy uav.launch'
  'MRS_control' 'waitForRos; roslaunch mrs_uav_manager f550_optflow.launch
'
	'MotorsOn' 'rosservice call /'"$UAV_NAME"'/control_manager/motors 1'
	'Takeoff' 'rosservice call /'"$UAV_NAME"'/uav_manager/takeoff'
	'Pirouette' 'rosservice call /'"$UAV_NAME"'/control_manager/pirouette'
	'Land' 'rosservice call /'"$UAV_NAME"'/uav_manager/land'
  'TF' 'waitForRos; rosrun tf2_ros static_transform_publisher 0.21 0 -0.03 0 0 0 fcu_uav5 rs_d435_link
'
